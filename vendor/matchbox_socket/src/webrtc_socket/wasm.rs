...
impl Messenger for WasmMessenger {
    ...
    async fn offer_handshake(
        ...
        // ARMY-GHOSTS PATCH (true trickle ICE): upstream waited for ICE
        // gathering to COMPLETE here so all candidates ride inside the offer
        // SDP. On multi-interface machines (VPN/tailscale utuns) browsers take
        // the full ~40s STUN timeout per handshake leg before reporting
        // complete, so connects took 80s+. Instead, send the offer
        // immediately and trickle candidates as they arrive (handler installed
        // below); the remote buffers early candidates while awaiting the
        // offer/answer (see the signal loops), so none are lost.
        start_candidate_trickle(&conn, signal_peer.clone());
    ...
    async fn accept_handshake(
        ...
        // ARMY-GHOSTS PATCH (true trickle ICE): see offer_handshake — send the
        // answer immediately and trickle candidates instead of stalling on
        // gathering-complete (~40s on multi-interface machines).
        start_candidate_trickle(&conn, signal_peer.clone());
...
/// ARMY-GHOSTS PATCH: forward ICE candidates to the remote peer as they are
/// gathered (true trickle), starting BEFORE the offer/answer is sent. Same
/// closure body as the one `complete_handshake` installs (which later replaces
/// this one — candidates fire once, so nothing is sent twice).
fn start_candidate_trickle(conn: &RtcPeerConnection, signal_peer: SignalPeer) {
    let onicecandidate: Box<dyn FnMut(RtcPeerConnectionIceEvent)> = Box::new(
        move |event: RtcPeerConnectionIceEvent| {
            let candidate_json = match event.candidate() {
                Some(candidate) => js_sys::JSON::stringify(&candidate.to_json())
                    ...
                    .unwrap(),
                None => {
                    debug!("no further ice candidates for this session (pre-handshake)");
                    "null".to_string()
                }
            };
            debug!("trickling IceCandidate signal: {candidate_json:?}");
            signal_peer.send(PeerSignal::IceCandidate(candidate_json));
        },
    );
    let onicecandidate = Closure::wrap(onicecandidate);
    conn.set_onicecandidate(Some(onicecandidate.as_ref().unchecked_ref()));
    // Leaked deliberately (one small closure per handshake); replaced by
    // complete_handshake's own handler once the handshake finishes.
    onicecandidate.forget();
}
